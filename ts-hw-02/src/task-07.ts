...
// login и верни true или false в зависимости от того, попадает ли длина параметра в з
// аданный диапазон от 4 - х до 16 - ти символов включительно.
...
type isLoginValidFn = (login: string) => boolean;
const isLoginValid: isLoginValidFn = function (login) {
  return login.length >= 4 && login.length <= 16;
};

type isLoginUniqueFn = (allLogins: string[], login: string) => boolean;
const isLoginUnique: isLoginUniqueFn = function (allLogins, login) {
  return !allLogins.includes(login);
};

type addLoginFn = (allLogins: string[], login: string) => string;
const addLogin: addLoginFn = function (allLogins, login) {
  let message: string;
